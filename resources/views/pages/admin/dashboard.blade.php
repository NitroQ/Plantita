...
<section>
  ...
  <div class="grid grid-cols-5 gap-5">
    ...
    <div class="lg:col-span-4 col-span-5 space-y-5">
      <div class="shadow-leftBottom p-5 rounded-md space-y-4 h-full">
        <span>
    ...
    <div class="lg:col-span-1 col-span-5 space-y-5">
      ...
      <div class="shadow-leftBottom p-5 rounded-md space-y-3">
        ...
        <div class="space-y-2">
          <button class="text-lg bg-background w-full py-2 hover:bg-green-200/10 rounded"><a href="{{ route('admin.transactions.index') }}">Check Transactions</a></button>
        ...
        @foreach($pendingTransactions as $pendingTransaction)
          @if($counter < 10)
            <div class="py-2 px-5 flex justify-between">
              <div>
                <p class="leading-6 text-lg capitalize">{{ $pendingTransaction->name }}</p>
                <p class="leading-6 text-lg text-gray-500">#{{ $pendingTransaction->id }}</p>
              </div>
              <p class="leading-6 text-lg font-brandon-black">₱{{ number_format($pendingTransaction->total, 2) }}</p>
            </div>
