    ...
    <div class="col-span-5 lg:col-span-2 space-y-8">
      <div class="col-span-5 lg:col-span-2 shadow-leftBottom p-5 rounded-md">
        <div class="flex justify-between mb-3">
          ...
          <span class="text-sm bg-green-400/20 text-green-400 uppercase font-brandon-black px-3 py-2 rounded">Done</span>
          @elseif($t->status == "cancelled")
          <span class="text-sm bg-orange/20 text-orange uppercase font-brandon-black px-3 py-2 rounded">Cancelled</span>
          @endif
        </div>
        <div class="grid grid-cols-5 gap-y-3 gap-x-5 items-start text-lg">
          ...
          <p class="col-span-3 mt-1">{{ $t->ship_method }}</p>
      ...
      <div class="space-y-3">
        <div class="grid grid-cols-3 space-x-3 pt-3">
            <a type="button" href="{{ route('transaction.status',['id' => $t->id, 'status' => 'cancelled']) }}" class="bg-crimson text-white text-lg py-2 rounded hover:bg-crimson/80"><i
                    class='bx bx-x-circle me-3'></i>Cancel Order</a>
            <button class="bg-blue-100 text-white text-lg py-2 rounded hover:bg-blue-100/80"><i
                    class='bx bx-printer me-3'></i>Print Waybill</button>
            <a type="button" href="{{ route('transaction.status',['id' => $t->id, 'status' => 'packed']) }}" class="bg-yellow-400 text-lg py-2 rounded hover:bg-yellow-400/80"><i class='bx bx-package me-3'></i>Item Prepared</a>
        </div>
    </div>
    @elseif($t->status == "packed")
      <form action="{{ route('transaction.pack',[$t->id]) }}" method="POST" class="space-y-3">
        ...
        <input id="shipment" type="text" name="courier" class="block w-full col-span-2 rounded-lg border border-lavender px-3.5 py-2.5 focus:ring-green-200/20 focus:border-green-200/20 placeholder-gray-500/70" placeholder="Choose Courier">
        <span class="text-sm text-crimson my-1">{{$errors->first('courier')}}</span>
        <input id="shipment" type="text" name="courier_location" class="block w-full col-span-2 rounded-lg border border-lavender px-3.5 py-2.5 focus:ring-green-200/20 focus:border-green-200/20 placeholder-gray-500/70" placeholder="Courier Location">
        <span class="text-sm text-crimson my-1">{{$errors->first('courier_location')}}</span>
        <input id="shipment" type="text" name="shipping_id" class="block w-full col-span-2 rounded-lg border border-lavender px-3.5 py-2.5 focus:ring-green-200/20 focus:border-green-200/20 placeholder-gray-500/70" placeholder="Tracking/Reference No.">
